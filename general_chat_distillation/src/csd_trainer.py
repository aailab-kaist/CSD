...
class CSDTrainer(Trainer):
    ...
    def kd_loss(
        ...
        if self.loss_type == "dpkd":
        ...
        elif self.loss_type in ["csd_ss_ss"]:
            if self.gradual_beta:
                beta = 1.0 + 0.5 * min(1., 2 * self.state.global_step / self.state.max_steps)
            else:
                beta = 1.0
            ...
            raise ValueError(
                f"Unknown loss type: {self.loss_type}. Should be one of ['dpkd', 'distillm_v1', 'distillm_v2', 'gkd', 'kd', 'seqkd','csd']"
            )
    ...
    def get_batch_logps(
        ...
        elif "csd" in loss_type:
            torch.cuda.empty_cache()

            if "ss_ss" in loss_type:  # Teacher(ss)_Student(ss)
                tea_pos_kl = (logits - tea_logits - torch.sum(vocab_logps.exp() * (logits - tea_logits), dim=-1, keepdim=True)).detach() * vocab_logps.exp().detach() * logits
                ref_pos_kl = (logits - tea_logits - torch.sum(vocab_logps.exp() * (logits - tea_logits), dim=-1, keepdim=True)).detach() * vocab_logps.exp().detach() * logits

                tea_pos_kl = tea_pos_kl.sum(-1)
                ref_pos_kl = ref_pos_kl.sum(-1)

            else:
                raise ValueError(f"Unknown CSD loss type: {loss_type}")
