...
from csd import forward_kl, reverse_kl, js_distance, tv_distance, csd, csd_weighting_function, dld_ablation
from csd import skewed_forward_kl, skewed_reverse_kl
from csd import SampleGenerator, ReplayBuffer
...
def get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits, epoch= None, total_epoch = None):
    with torch.no_grad():
        teacher_model.eval()
    ...
    if args.model_parallel:
        ...
        elif "csd_temperature_ablation" in args.type:
            distil_loss = csd_temperature_ablation(logits, teacher_logits, no_model_batch, temp=args.loss_temp)
        elif "csd_weighting_function" in args.type:
            distil_loss = csd_weighting_function(logits, teacher_logits, no_model_batch, type=args.abl_type, epoch= epoch, total_epoch = total_epoch)
        elif "csd" in args.type:
            distil_loss = csd(logits, teacher_logits, no_model_batch, temp=args.loss_temp)
        elif "dld_ablation" in args.type:
            distil_loss = dld_ablation(logits, teacher_logits, no_model_batch, normalize=args.normalize, w=args.weighting, temp=args.loss_temp)
        else:
            raise NotImplementedError
    return distil_loss
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if teacher_model is not None:
                distil_loss = get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits, epoch=epoch, total_epoch=args.epochs)
