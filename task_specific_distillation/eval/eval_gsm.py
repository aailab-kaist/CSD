...
@click.option('-ckpt', type=str, default='./ckpt/teacher/gemma-7b-it-gsm_sft')
def main(max_tokens, visualize_text, ckpt):
  vali_dataset = load_dataset('openai/gsm8k', 'main')['test']
  tokenizer = AutoTokenizer.from_pretrained('google/gemma-7b-it') #'google/gemma-7b-it'
  vali_data_inp = [
    ...
    if '####' in ele.outputs[0].text:
      ...
      try:
        if r'\pi' in ans or r'\pi' in gt:
          ...
          equiv = math_equal(ans, gt, tolerance=1e-3, timeout=True)
