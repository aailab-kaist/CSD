...
class CSD_TS_mix(nn.Module):
  def __init__(self, alpha):
    super(CSD_TS_mix, self).__init__()
    self.alpha = alpha
  def forward(self, logits, teacher_logits):  
      teacher_probs = F.softmax(teacher_logits, dim=-1, dtype=torch.float32)
      student_probs = F.softmax(logits, dim=-1, dtype=torch.float32)
      prob = self.alpha * teacher_probs + (1-self.alpha) * student_probs
      loss = (logits - teacher_logits - torch.sum(prob * (logits - teacher_logits), dim=-1, keepdim=True)).detach() * prob.detach() * logits
      x = torch.sum(loss, dim=-1).view(-1)
      distil_loss = torch.mean(x, dim=0)
      return distil_loss

class CSD_TS(nn.Module):
  def __init__(self):
    super(CSD_TS, self).__init__()

  def forward(self, logits, teacher_logits):  # torch.Size([121, 256000]), torch.Size([121, 256000])
      teacher_probs = F.softmax(teacher_logits, dim=-1, dtype=torch.float32)
      student_probs = F.softmax(logits, dim=-1, dtype=torch.float32)
...
def compute_vali_loss(vali_data_src, vali_data_ref, assistant_model, tokenizer,
  ...
  adev = next(assistant_model.parameters()).device
  mdev = next(model.parameters()).device if model is not None else None
...
@click.option("-wandb_proj", type=str, help="project_name")
...
def main(top_k, top_p, task_type, grad_acc_size, kd_type, distance_metric,
         ...
         mixed_ratio, expected_seq_len, alpha):
  if seed:
    set_seed(seed)
    random.seed(seed)
  ...
  if enable_stop_token:
    if task_type == "gsm_1k" or task_type == "gsm_100" or task_type =='gsm_7k':
      end_of_string_ls = ["\n\n"]
  ...
  print("assistant_checkpoint: ",assistant_checkpoint)
  ...
  base_assistant = accelerator.unwrap_model(assistant_model) if hasattr(accelerator, "unwrap_model") else getattr(assistant_model, "module", assistant_model)
  adev = next(base_assistant.parameters()).device
  ...
  if kd_type != "seq_kd":
    ...
    mdev = next(base_model.parameters()).device


  if kd_type == "skd" or kd_type == "mixed_skd":

    base_model.generation_config.num_assistant_tokens = 5
    base_model.generation_config.num_assistant_tokens_schedule = "constant"
    base_model.generation_config.num_beams = 1         
    base_model.generation_config.do_sample = True     
    base_model.generation_config.top_p = student_top_p
    base_model.generation_config.temperature = student_temperature
  ...
  if kd_type == "seq_kd":
    ...
    elif distance_metric == "csd_ts":
      loss_funct = CSD_TS()
    elif distance_metric == "csd_ts_mix":
      loss_funct = CSD_TS_mix(alpha)
    else:
  ...
  if not debug_enable and accelerator.is_main_process:
    ...
    wandb.init(project=wandb_proj,
               ...
               f"jsd_beta_{jsd_beta}_{ckpt_prefix}_{lr}", config={
                   "lr": lr,
  ...
  save_inp_data_file = open(
      f"txt_file/{task_type}_seed_{seed}_{kd_type}_{distance_metric}_{top_k}_"
      f"{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}_{lr}.input", "w"
  )
  save_out_data_file = open(
      f"txt_file/{task_type}_seed_{seed}_{kd_type}_{distance_metric}_{top_k}_"
      f"{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}_{lr}.output", "w"
  )
    ...
    with tqdm(total=len(list(train_data_src_tot))*n_epoch) as pbar:
      for cur_epoch_index in range(n_epoch):
        ...
        for prompts in prompt_batchify(train_data_src_tot, grad_acc_size):
          temp_report_dict = {}
          batch_loss = 0
          if (accelerator.is_main_process and all_steps % eval_step == 0 and all_steps != 0):

            if kd_type == "seq_kd":
              vali_loss = compute_vali_loss(
                  vali_data_src, vali_data_ref, base_assistant, 
              ...
              vali_loss = compute_vali_loss(
                  vali_data_src, vali_data_ref, base_assistant, 
                  tokenizer, loss_funct, base_model, inp_length,
                  max_length, loss_type
              )
            temp_report_dict['vali_loss'] = vali_loss

            if kd_type == "skd" or kd_type == "mixed_skd":
              save_name = f'./ckpt/{prefix}/{task_type}_seed_{seed}_{kd_type}_{distance_metric}_{top_k}_{all_steps}_{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}_{lr}'
            else:
              save_name = f'./ckpt/student/distill/{prefix}/{task_type}_seed_{seed}_{kd_type}_{distance_metric}_{all_steps}_{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}_{lr}'
          ...
          for prompt in prompts:
            with torch.no_grad():
              if kd_type == "on-policy":
                ...
                final_outputs = base_assistant.generate( 
                    **inputs,
                ...
                final_outputs = base_model.generate( 
                    **inputs,
                    max_new_tokens=max_new_tokens,
                    do_sample=True,
                    num_beams=1,
                ...
                if rand_index == 0:
                  final_outputs = base_model.generate( 
                      **inputs, 
                      assistant_model=base_assistant, 
                      max_new_tokens=max_new_tokens,
                      do_sample=True,
            ...
            student_inputs = {k: (v.to(adev, non_blocking=True) if torch.is_tensor(v) else v)
                  for k, v in final_outputs.items()}
            student_outputs = assistant_model(**student_inputs) 
            ...
            if kd_type == "seq_kd":
              ...
              if "csd" in distance_metric:
                shift_student_logit = (student_outputs.logits[0, inputs.input_ids.shape[1]:].contiguous())
                teacher_inputs = {k: (v.to(mdev, non_blocking=True) if torch.is_tensor(v) else v)  for k, v in final_outputs.items()}  
                teacher_outputs = model(**teacher_inputs) 
                shift_teacher_logit = (teacher_outputs.logits[0, inputs.input_ids.shape[1]:].contiguous())

              else:
                shift_student_logit = (student_outputs.logits[0, inputs.input_ids.shape[1]:].contiguous())
                shift_student_logps = (student_outputs.logits[0, inputs.input_ids.shape[1] :].log_softmax(dim=-1).contiguous())
                teacher_inputs = {k: (v.to(mdev, non_blocking=True) if torch.is_tensor(v) else v) for k, v in final_outputs.items()} 
                teacher_outputs = model(**teacher_inputs) 
                shift_teacher_logps = (teacher_outputs.logits[0, inputs.input_ids.shape[1] :].log_softmax(dim=-1).contiguous())
              if "csd" in distance_metric:
                loss = loss_funct(shift_student_logit, shift_teacher_logit)
